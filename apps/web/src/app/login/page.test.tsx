...
describe("LoginPage (direct-link route)", () => {
  ...
  it("redirects an existing tenant session to /dashboard", async () => {
    mockGetSession.mockResolvedValue({
      ok: true,
      data: {
        user: {
        ...
        memberships: [{ id: "tenant-membership-test" }],
  ...
  it("redirects to /platform-admin if a valid session already exists", async () => {
    mockGetSession.mockResolvedValue({
      ok: true,
      data: {
        user: {
    ...
    await waitFor(() => {
      expect(mockReplace).toHaveBeenCalledWith("/platform-admin");
    });
  });
  ...
  it("redirects a tenant user to /dashboard on successful login", async () => {
    const user = userEvent.setup();
    mockLogin.mockResolvedValue({
      ok: true,
      data: {
  ...
  it("redirects to /platform-admin on successful login", async () => {
    const user = userEvent.setup();
    mockLogin.mockResolvedValue({
      ok: true,
      data: {
